...
**Skill 自动加载：** J.A.R.V.I.S. 检测到工程类或设计类需求时，会自动调用 `Skill` 工具将对应 Skill 加载到当前对话，然后利用 Skill 的知识完成任务。例如：提到"代码审查"自动加载 `code-review-and-quality`，提到"调试"自动加载 `debugging-and-error-recovery`，提到"安全审查"自动加载 `security-and-hardening`。
...
本系统配套了 26 个工程 Skills（位于 `~/.claude/skills/`），覆盖开发全流程：

| Skill | 用途 | 自动加载条件 |
|-------|------|-------------|
| `code-review-and-quality` | 多维代码审查（正确性/可读性/架构/安全/性能） | 提到代码审查、PR 审查等 |
| `debugging-and-error-recovery` | 系统化调试流程 | 提到调试、构建失败、测试失败等 |
| `security-and-hardening` | 代码安全加固检查 | 提到安全审查、漏洞检查等 |
| `performance-optimization` | 性能分析优化 | 提到性能、响应慢、优化等 |
| `incremental-implementation` | 增量实现，小步快跑 | 提到逐步实现、分步完成等 |
| `planning-and-task-breakdown` | 任务拆解与规划 | 提到任务拆分、执行计划等 |
| `shipping-and-launch` | 发布上线检查清单 | 提到发布、上线、部署等 |
| `git-workflow-and-versioning` | Git 工作流规范 | 提到 Git、分支策略、版本管理 |
| `test-driven-development` | 测试驱动开发 | 提到测试、TDD 等 |
| `ci-cd-and-automation` | CI/CD 流水线 | 提到 CI/CD、自动化构建等 |

> 完整 26 个技能列表请查看 [Claude Skills 仓库](https://github.com/mjzappzz/claude-skills)。
...
**相关资源：**
- [Claude Skills 仓库](https://github.com/mjzappzz/claude-skills) — 26 个工程技能（调试/审查/CI/CD/安全/性能优化等）
- [Codex Skills 仓库](https://github.com/mjzappzz/codex-skills) — 26 个工程技能（Codex 版本，与本仓库同步）
